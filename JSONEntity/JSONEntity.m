...
+ (NSArray *)getPropertyNames:(id)obj{
    ...
    Class cls = [obj class];
    while (cls) {
        if ([NSStringFromClass(cls) isEqualToString:@"NSObject"]) break;
...
+ (instancetype)fromDict:(NSDictionary *)d map:(NSDictionary *)map{
    return [[[self alloc] init] fromDict:d map:map];
}

+ (instancetype)fromJSON:(NSData *)json map:(NSDictionary *)map{
    return [self fromDict:[NSJSONSerialization JSONObjectWithData:json options:NSJSONReadingMutableContainers error:nil] map:map];
}
...
- (Class)_mapedClass:(NSString *)key map:(NSDictionary *)map{
    if (map[key]){
        ...
        NSString *c  = [key substringToIndex:1];
        //return NSClassFromString([key capitalizedString]);
        return NSClassFromString([key stringByReplacingCharactersInRange:NSMakeRange(0, 1) withString:[c uppercaseString]]);
    }
}
